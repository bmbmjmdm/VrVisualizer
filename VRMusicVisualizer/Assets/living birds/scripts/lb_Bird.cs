...
public class lb_Bird : MonoBehaviour {
    ...
    void Start() {
		// My own custom flying mechanics
		...
		// pick destination
		target = new Vector3(gameObject.transform.position.x + 10f*xDir, gameObject.transform.position.y, gameObject.transform.position.z + 10f*zDir);
	...
	void Update () {
        ...
        if (clock >= 0.02) {
			...
			if (Vector3.Distance (gameObject.transform.position, target) <= 0.1f) {
				...
				gameObject.transform.position += new Vector3(xDir*0.07f, 0, zDir*0.07f);
			}
		}
	...
	IEnumerator FlyToTarget(Vector3 target){
		...
		GetComponent<Rigidbody>().AddForce((transform.forward * 50.0f*birdScale)+(transform.up * 100.0f*birdScale));
		float t = 0.0f;
		while (t<1.0f){
			if(!paused){
				t+= Time.deltaTime;
		...
		if(vectorDirectionToTarget.y>.5f){
			...
			while(vectorDirectionToTarget.y>.5f){
				...
				GetComponent<Rigidbody>().AddForce(transform.forward * 70.0f*birdScale * Time.deltaTime);
				...
				if (Physics.Raycast(transform.position,-Vector3.up,out hit,0.15f*birdScale) && GetComponent<Rigidbody>().velocity.y < 0){
		...
		while(true){
			if(!paused){
				...
				if (Physics.Raycast(transform.position,-Vector3.up,0.15f*birdScale) && GetComponent<Rigidbody>().velocity.y < 0){
					GetComponent<Rigidbody>().velocity = new Vector3(GetComponent<Rigidbody>().velocity.x, 0.0f,GetComponent<Rigidbody>().velocity.z);
				}
				if (Physics.Raycast(transform.position,Vector3.up,out hit,0.15f*birdScale) && GetComponent<Rigidbody>().velocity.y > 0){
					//if the bird is going to collide with something overhead zero out vertical velocity
